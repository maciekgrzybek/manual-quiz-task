...
export const getQuizInitialState = () => {
  const previousAnswers = quizStorage.getAnswers();
  const questionIndexes = previousAnswers
    ? Object.keys(previousAnswers).map(Number)
    : undefined;
  const previousQuestionIndex = questionIndexes
    ? Math.max(...questionIndexes)
    : undefined;
  /*
  ...
  return { questions, previousAnswers, previousQuestionIndex };
};
